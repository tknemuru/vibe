# Vibe Book Collector 改修 指示書（Claude Code 用・完全版）
...
3) 破壊操作の運用改善：
   - `vibe db reset`（DB全クリア）を実装
   - `vibe mail reset`（配信状態だけクリア）を実装
...
4) vibe mail reset: --since / --job の対象範囲が正しい
5) vibe db reset: bak作成 + 新規初期化が正しい
...
### 3) Copyページはローカル `vibe serve` で提供（MVP）
- `vibe serve --host 0.0.0.0 --port 8787` のように起動できる。
...
### 5) 運用改善コマンド
- `vibe db reset [--yes]`：DBを `.bak` に退避して再初期化
- `vibe mail reset [--job <name>] [--since <duration>] [--yes]`：
...
### Task 6: `vibe serve` 実装（Copyページ）
- `vibe serve` コマンドを追加し、HTTPサーバを起動できること。
- `GET /p/:token`：
  - token検証/期限チェック
  - prompt_textだけを表示
