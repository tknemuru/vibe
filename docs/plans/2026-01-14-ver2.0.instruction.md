...
* **CLI（dre）**：実行入口、設定管理、DB操作コマンド、run-due実行
...
既存 `dre` を「書籍収集＋DeepResearch支援メール」へ刷新し、
...
* **DoD**
  ...
  * `dre run-due` が新パスに入れる（中身がstubでも良い）
* **Verify**

  * `npm run build`
  * `dre --help` / `dre run-due --help`
* **Rollback**
...
* **Risks**
  ...
  * 既存DB利用者がいる場合は破壊的 → `dre db reset` で明示的に作り直せるようにする
...
* **Verify**
  ...
  * `dre run-due --force-mail` でDBにbooksが増える
* **Rollback**
...
* **コマンド仕様**
  ...
  2. `dre mail reset [--job <name>] [--since <duration>] [--yes]`
