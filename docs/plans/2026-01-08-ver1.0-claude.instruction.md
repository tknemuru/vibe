...
* CLI方式2
  ...
  * `dre feedback inbox`（未評価一覧）
  * `dre feedback good|bad <ID...>`（複数一括）
...
CLIアプリ `dre` を実装する。ジョブをYAML管理し、`run-due`でdueジョブのみ検索→新規抽出→LLM要約→HTMLメール送信→フィードバックをSQLiteに保存し次回ランキングへ反映する。無料枠厳守のため日次クエリ上限95でフェイルクローズする。
...
* **Steps**
  ...
  3. `.env` ロード（dotenv）
  4. `dre doctor`（設定診断）コマンド追加
* **DoD**

  * `dre --help` と `dre doctor` が動く
* **Verify**
...
* **成果物**
  ...
  * `dre job ls/add/update/rm/enable/disable`
* **Steps**
...
1. `dre doctor` が必須設定不足を検出し案内できる
2. `dre run-due` で検索→新規抽出→要約→HTMLメール送信が通る
3. `dre feedback inbox` で未評価が出る
4. `dre feedback good/bad <ID>` が保存され、次回の採用に影響する
